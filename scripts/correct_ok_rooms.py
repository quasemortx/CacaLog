...
def main():
    ...
    for room in correction_rooms:
        ...
        if target_item:
            ...
            logger.info(f"Correcting Room {room} (Status: {current_status}, Model: {current_model}) -> OK / Model Cleared")
            ...
            # Preserve
            if target_item.get("Predio"): new_item.predio = target_item.get("Predio")
            if target_item.get("Andar"): new_item.andar = target_item.get("Andar")
            if target_item.get("TipoAmbiente"):
                 try: new_item.tipo_ambiente = TipoAmbiente(target_item.get("TipoAmbiente"))
                 except: pass
