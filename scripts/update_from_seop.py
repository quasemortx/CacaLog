    ...
    for msg in messages:
        ...
        for new_data in extracted:
            ...
            if new_data.get("tipo_ambiente") == TipoAmbiente.LAB:
                if new_data.get("total_pcs") is not None:
                    item.total_pcs = new_data["total_pcs"]
                if new_data.get("concluidos") is not None:
                    item.concluidos = new_data["concluidos"]
                if new_data.get("pendentes") is not None:
                    item.pendentes = new_data["pendentes"]
                if new_data.get("erros") is not None:
                    item.erros = new_data["erros"]
