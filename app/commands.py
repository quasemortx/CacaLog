...
"""
    ...
    if cmd == "/planilha":
        url = f"https://docs.google.com/spreadsheets/d/{sheets.sheet.id}" if hasattr(sheets, 'sheet') else "Link não disponível."
    ...
    if cmd == "/status":
        if not args: return "Uso: /status [sala]"
        ...
        if subcmd in ["labs", "laboratorios", "laboratórios"]:
             ...
             if not found: return "Nenhum lab registrado."
        ...
        if not found: return f"Não encontrado: {target}"
        resp = ""
        for f in found:
    ...
    if cmd == "/lab":
        if not args: return "Uso: /lab [numero]"
        raw_arg = args[0]
        
        # Try variations: "9" -> "L-9", "L-09", "LAB 9", "LAB 09"
        targets = [
            f"L-{raw_arg}", 
            f"L-{int(raw_arg):02d}" if raw_arg.isdigit() else raw_arg
        ]
        ...
        if not found: return "Lab não encontrado."
        
        resp = ""
        for r in found:
             resp += f"{r['local_id']} ({r['TipoAmbiente']}): {r['Status']}\nObs: {r['Observacao']}\n"
        return resp.strip()

    if cmd in ["/pendentes", "/erros", "/incompativeis", "/atualizando", "/subindo", "/naoavaliados", "/faltantes"]:
        target_status = {
            "/pendentes": "PENDENTE",
            "/erros": "ERRO",
        ...
        if not found: return f"Nenhum registro com status {target_status}."
    ...
    if cmd in ["/ti", "/manutencao", "/manutenção"]:
        ...
        if not found: return f"Nenhum chamado pendente para {target_sector}."
    ...
    if cmd == "/resumo":
        if not args: return "Uso: /resumo p1 ou p2"
        predio_arg = args[0].lower()
        if predio_arg not in ['p1', 'p2']: return "Prédio inválido."
        ...
        def format_breakdown(total, ti, man):
            if total == 0: return ""
            parts = []
            if ti > 0: parts.append(f"{ti} TI")
            if man > 0: parts.append(f"{man} Manut.")
            if not parts: return ""
            return f" ({' / '.join(parts)})"
        ...
        for r in filtered_all:
            is_lab = (r.get('TipoAmbiente') == 'LAB' or 
                     str(r.get('local_id', '')).upper().startswith('L-'))
            if is_lab:
                labs.append(r)
            else:
                salas.append(r)
        ...
        def generate_block(title, data):
            if not data: return "" # Don't show empty block
            
            t = len(data)
            o = len([r for r in data if r['Status'] == 'OK'])
            p_list = [r for r in data if r['Status'] == 'PENDENTE']
            pt = len(p_list)
            p_str = format_breakdown(pt, count_sector(p_list, "TI"), count_sector(p_list, "Manutenção"))
            
            e_list = [r for r in data if r['Status'] == 'ERRO']
            et = len(e_list)
            e_str = format_breakdown(et, count_sector(e_list, "TI"), count_sector(e_list, "Manutenção"))
        ...
        def generate_lab_block(title, data):
            ...
            return (f"{title} (Locais: {t_labs} | PCs: {total_pcs}):\n"
                    f"✅ OK: {ok_pcs}\n"
        ...
        if labs:
            resp += generate_lab_block("🧪 **Laboratórios**", labs)
             ...
             if p_num == 1: resp += "🧪 Labs (<20): Nenhum.\n"
             else: resp += "🧪 Labs (20+): Nenhum.\n"
        
        resp += f"\n💻 **Total Máquinas no Prédio:** {total_pcs_building}"
             
        return resp.strip()

    if cmd == "/labs" or (cmd == "/status" and args and args[0].lower() in ["labs", "laboratorios"]):
        found = [r for r in records if r['TipoAmbiente'] == 'LAB']
        if not found: return "Nenhum lab registrado."
    ...
    if cmd == "/andar":
        if len(args) < 2: return "Uso: /andar [andar] [predio] (Ex: /andar 7 p1)"
        ...
        if 'p1' in args[0].lower() or 'p2' in args[0].lower():
            p_str = args[0].lower()
            try: andar_val = int(args[1])
            except: pass
        elif 'p1' in args[1].lower() or 'p2' in args[1].lower():
            p_str = args[1].lower()
            try: andar_val = int(args[0])
            except: pass
        ...
        if not filtered: return f"Nenhum registro encontrado para Prédio {predio}, Andar {andar_val}."
