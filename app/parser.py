...
def validate_sala(numero_str: str) -> Optional[Dict]:
...
ROOM_KEYWORDS = {'sala', 'turma', 'aula', 'andar', 'p1', 'p2', 'prédio', 'predio', 'laboratório', 'lab'}
...
def get_status_from_text(text: str) -> Optional[Status]:
    """
    ...
    # Check Incompatible
    if any(x in lower_text for x in ['nao suporta', 'sem tpm', 'incompativel', 'incompatível', '390', 'legado']):
        return Status.INCOMPATIVEL
        
    # Check Error
    if any(x in lower_text for x in ['erro', 'falha', 'defeito', 'problema', 'tela azul', 'sem espaco', 'sem espaço', 'bug', 'bugada', 'bios', 'boot', 'travado', 'driver']):
        return Status.ERRO
        
    # Check Updating
    if any(x in lower_text for x in ['atualizando', 'verificando', 'subindo', 'clonando', 'fazendo', 'formatando', 'pancando', 'pankando']):
    ...
    # Check Pendente
    if any(x in lower_text for x in ['pendente', 'faltando', 'sem', 'instalar', 'configurar', 'hd', 'sem net', 'aguardando', 'rever', 'pendencia', 'pendência']):
        return Status.PENDENTE
        
    # Check OK
    if any(x in lower_text for x in ['ok', 'pronto', 'funcionando', 'normal', 'feito', 'instalado', 'concluido', 'concluído', 'resolvido', 'resolvidos']):
        return Status.OK
...
def extract_data_regex(message: str) -> List[Dict[str, Any]]:
    ...
    found_types = []
    if 'xps' in lower_msg: found_types.append("XPS")
    if 'optiplex' in lower_msg: found_types.append("OPTIPLEX")
    if 'hp' in lower_msg: found_types.append("HP")
    if 'alugada' in lower_msg or 'locada' in lower_msg: found_types.append("ALUGADA")
    if 'lenovo' in lower_msg:
        if 'mini' in lower_msg: found_types.append("LENOVO_MINI")
        elif 'lenovo' not in found_types: found_types.append("LENOVO") # Avoid dupe if mini logic complex, simplified here
        ...
        if 0 <= val <= 29:
            ...
            if len(lines) > 1:
                ...
                for line in lines:
                    ...
                    is_any_lab_header = bool(re.search(r'\b(?:lab(?:oratori[oa])?\.?\s*|l-)\s*\d+\b', line_lower))
            ...
            has_counts = False
            total_match = re.search(r'(\d+)\s*(?:m[aá]quinas|pcs|computadores)', text_segment, re.IGNORECASE)
            if total_match:
            ...
            ok_match = re.search(r'(\d+)\s*(?:[-–|]\s*)?(?:pcs?)?\s*(?:est[ãa]o)?\s*(?:ok|w11|win\s*11|concluid[oa]s?|feita?s?)', text_segment, re.IGNORECASE)
            if ok_match: item["concluidos"] = int(ok_match.group(1))

            pend_match = re.search(r'(\d+)\s*(?:[-–|]\s*)?(?:pcs?)?\s*(?:est[ãa]o)?\s*(?:pendentes?|faltan?do|sem|a fazer|win\s*10)', text_segment, re.IGNORECASE)
            if pend_match: 
            ...
            erro_match = re.search(r'(\d+)\s*(?:[-–|]\s*)?(?:pcs?)?\s*(?:est[ãa]o)?\s*(?:erro|defeito|falha|tela azul|sem espaco|sem espaço)', text_segment, re.IGNORECASE)
            if erro_match: item["erros"] = int(erro_match.group(1))
            
            if has_counts:
                # Infer status based on counts
                if item.get("pendentes", 0) > 0 or item.get("erros", 0) > 0:
                    item["status"] = Status.PENDENTE
                elif item.get("concluidos", 0) >= item.get("total_pcs", 0) and item.get("total_pcs", 0) > 0:
                    item["status"] = Status.OK
                elif item.get("concluidos", 0) > 0 and item.get("total_pcs", 0) == 0: # If only concluidos is mentioned
                    item["status"] = Status.OK
                else:
...
def normalize_status(status_raw: str) -> Status:
    ...
    if 'OK' in s or 'FEITO' in s or 'RESOLVIDO' in s: return Status.OK
    if 'PENDENTE' in s: return Status.PENDENTE
    if 'ERRO' in s: return Status.ERRO
    if 'INCOMPATIVEL' in s: return Status.INCOMPATIVEL
    if 'ATUALIZANDO' in s or '🔄' in s: return Status.ATUALIZANDO
